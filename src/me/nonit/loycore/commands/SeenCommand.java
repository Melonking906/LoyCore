...
public class SeenCommand implements CommandExecutor
{
    ...
    public SeenCommand( LoyCore plugin )
    {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand( CommandSender sender, Command cmd, String label, String[] args )
    {
        if ( ( sender instanceof Player ) )
        {
            if( args.length > 0 )
            {
                ...
                if( search.length() < 3 )
                {
                    sender.sendMessage( LoyCore.getPfx() + ChatColor.RED + "Please search with 3 or more letters!" );
                    return true;
                }
            ...
            {
                sender.sendMessage( LoyCore.getPfx() + "Look up a player with /seen <name>!" );
            }
        }
        return true;
    }
